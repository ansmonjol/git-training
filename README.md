...
All changes must happen on the dev branch, and your goal is to finish with exactly 6 commits on 2 different branches.

## Instructions

You're working on a project with coworkers, and pushing all your changes into a `dev` branch.
The goal is to prepare a PR to merge `dev` into `main` but you noticed the commit list is kinda messy and not clear.
...
Here you'll aims to preform multiple actions in order to make `dev` branch "cleaner" so it could be merged it into `main` branch.
...
## To go further
You can challenge yourself and try to execute all 1 to 7 actions within a single interactive rebase action.
